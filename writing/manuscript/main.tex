...
\begin{figure}[hbt!]
    ...
    approach peak near the max-water-level phase. (b) For many
    steady-state (sealed, overflowing) cases spanning the tidal swing $\Delta w$
    and the depth (colour), the approach-to-widening amplitude ratio --- i.e.\ the
    main/secondary peak ratio --- versus the widening-peak emission (per-length
    flux $\times$ 500~km). The cases form a locus that brackets the approximate
    observed Enceladus scenario (red; main/secondary $\approx$2 at an emission of
    a few hundred kg/s --- an estimate, since the absolute secondary-peak flux is
