...
\newcommand{\FITDW}{25}
\newcommand{\FITL}{20}
\newcommand{\FITPHI}{262}
\newcommand{\FITCHI}{0.98}
\newcommand{\FITCHISINGLE}{1.1}
...
attractor fixes the effective width at the overflow seal depth, leaving the
absolute tidal swing $\Delta w$, the effective source depth $L$, a single global
phase offset $\phi_0$, an amplitude scale, and --- through the second-harmonic
forcing introduced above --- the $2f$ amplitude $\alpha$ and phase $\phi_2$ as
free parameters; the amplitude absorbs the unknown slab-density-to-emission
proportionality, so only the profile \emph{shape} (peak phases, peak ratio,
trough depth) constrains the physics. A pure single-cosine forcing ($\alpha=0$)
already fits reasonably (reduced $\chi^2=\FITCHISINGLE$), but admitting the second
harmonic improves the fit at every depth and yields the best profile
(Figure~\ref{fig:enc:fit}): it reproduces the sharp main peak, the broad
secondary shoulder, the steep post-peak decline, and the deep trough with a
reduced $\chi^2 = \FITCHI$, for a tidal swing $\Delta w \approx \FITDW$~mm, an
effective source depth $L \approx \FITL$~km, a second-harmonic amplitude
$\alpha \approx 1.5$ (phase $\phi_2\approx45^\circ$), and a global phase offset
$\phi_0 \approx \FITPHI^\circ$. That a substantial $2f$ term is favoured is
itself consistent with the multi-harmonic character of the tidal stress that
drives the walls \cite{nimmo2014tidally}.

The formal optimum lies at the deep, large-swing end of the explored range, so
the depth is best read as a bound, $L \gtrsim \FITL$~km, rather than a pinned
value; the acceptable region (reduced $\chi^2 \lesssim 1.3$) spans
$L\approx10$--$20$~km. This deep, approach-peak-dominated regime is what the
observed profile --- a single strong main peak with only a weak, broad secondary
(amplitude ratio ${\sim}2$--$3$) --- selects, rather than the two-comparable-peak
regime of a very shallow crack (Section~\ref{sec:peaks}). The shallower end of the
acceptable range ($L\approx10$--$14$~km) is compatible with the reduced ice-shell
thickness inferred for the south-polar terrain from gravity and topography, so the
fit does not require a globally thick shell even though its formal minimum is deep.
Two caveats bound the inference: the target is a digitized central-trend curve
through a wide, multi-year scatter (so the weak secondary is poorly constrained),
and the amplitude is a free scale, so the fit is driven by the profile shape and
relative peak heights, not the absolute emission.

\placeholder{Develop the implications of the fitted parameters: (i) reconcile the
deep formal optimum ($L\gtrsim20$~km) with the shallow-source reading of the
Conclusions and with the reduced south-polar shell thickness --- add the relevant
ice-shell-thickness references (e.g. gravity/topography inversions of the SPT);
(ii) interpret the favoured second-harmonic amplitude in terms of the tidal-stress
harmonic content. Asserting nothing here beyond ``compatible with'' until these
references and the depth reconciliation are settled.}

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.7\textwidth]{Figures/diurnal_fit.pdf}
    \caption{Best-fit model diurnal mass-flux profile (red) against the digitized
    ...
    second-harmonic (double-cosine) amplitude and phase, a single global phase
    offset, and an overall amplitude. The double-cosine forcing improves the fit
    at every depth over a pure single cosine. The sharp main peak is the
    single-crack rapid-surge onset --- the water reaching the surface at speed ---
    which is smoothed once many crack segments are averaged (Figure~\ref{fig:enc:ensemble}).}
    \label{fig:enc:fit}
\end{figure}
...
comparison to the spatially-integrated data. Fitting $\sigma_\phi$ to the
single-crack best fit rounds the peak and markedly improves the agreement, from
reduced $\chi^2\approx0.98$ (single crack) to $\approx0.52$ for
$\sigma_\phi\approx24^\circ$ (Figure~\ref{fig:enc:ensemble}); the ensemble peak
amplitude and width now match the observations without further tuning of the
crack parameters.
...
\begin{figure}[hbt!]
    ...
    the peak; averaging over the along-strike spread of tiger-stripe segments
    (a Gaussian phase spread $\sigma_\phi\approx24^\circ$; red) rounds the peak to
    match the observations, improving the reduced $\chi^2$ from $0.98$ to $0.52$.
    Crack parameters are held at the single-crack best fit of
    Figure~\ref{fig:enc:fit}; only the ensemble spread, phase offset, and
