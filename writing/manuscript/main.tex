...
\newcommand{\FITDW}{25}
\newcommand{\FITL}{20}
\newcommand{\FITPHI}{238}
\newcommand{\FITCHI}{1.1}
...
phase offset $\phi_0$, and an amplitude scale as free parameters; the amplitude
absorbs the unknown slab-density-to-emission proportionality, so only the profile
\emph{shape} (peak phases, peak ratio, trough depth) constrains the physics. The
best fit (Figure~\ref{fig:enc:fit}) reproduces the full profile --- the sharp
main peak, the broad secondary shoulder, the steep post-peak decline, and the
deep trough --- with a reduced $\chi^2 = \FITCHI$, for a tidal swing
$\Delta w \approx \FITDW$~mm and an effective source depth $L \approx \FITL$~km
(global phase offset $\phi_0 \approx \FITPHI^\circ$). The minimum lies at the
deep, large-swing end of the explored range, so these values are best read as a
\emph{deep} source ($L \gtrsim 20$~km) with a swing of a few tens of millimetres
rather than as a tightly pinned optimum.

The fit therefore selects a comparatively deep source. This is consistent with
the observed profile being dominated by a single sharp main peak with only a
weak, broad secondary (amplitude ratio ${\sim}2$--$3$): in the model this is the
approach-peak-dominated, high-ratio regime that Section~\ref{sec:peaks} associates
with greater depth, rather than the two-comparable-peak regime of a very shallow
crack. The corresponding effective width on the attractor is
$w_{\rm eff}\approx22$~mm, and the single global phase offset that aligns the model
is the only phase freedom used. Two caveats bound this inference: the target is a
digitized central-trend curve through a wide, multi-year scatter (so the weak
secondary is poorly constrained), and the amplitude is a free scale, so the fit
is driven by the profile shape and the relative peak heights, not by the absolute
emission.

\placeholder{This deep-source result should be reconciled with the shallow-source
reading in the Conclusions, which was based on the mere presence of two peaks; the
quantitative profile fit instead favours a deep, approach-dominated source.
Revisit and soften that claim once this fit is finalized.}
...
\begin{figure}[hbt!]
    ...
    parameters are the tidal swing $\Delta w$, effective source depth $L$, a
    single global phase offset, and an overall amplitude.}
    \label{fig:enc:fit}
\end{figure}
