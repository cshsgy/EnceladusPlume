...
% Prominence-weighted continuous MLE (exact best fit) + MCMC posterior; run_fit.py
\newcommand{\FITDW}{9}
\newcommand{\FITL}{7}
\newcommand{\FITTWOF}{3.5}
\newcommand{\FITSIG}{22}
\newcommand{\FITPHITWO}{82}
\newcommand{\FITLPOST}{8^{+1}_{-2}}
\newcommand{\FITCHI}{3.3}
\newcommand{\FITCHISINGLE}{7.2}
...
proportionality, so only the profile \emph{shape} constrains the physics. Because
the diagnostic feature is the \emph{relative strength} of the secondary peak
rather than its absolute level, we add to the (peak-weighted) least-squares
objective an explicit term matching the secondary's prominence --- its amplitude
above the adjacent inter-peak baseline --- and optimise by continuous maximum
likelihood (global differential-evolution search followed by local refinement).

Requiring the model to reproduce the observed secondary \emph{prominence} selects
a distinct solution (Figure~\ref{fig:enc:fit}): a \emph{shallow} source
($L\approx\FITL$~km) with a \emph{strong} second harmonic
($\alpha\approx\FITTWOF$, $\phi_2\approx\FITPHITWO^\circ$), tidal swing
$\Delta w\approx\FITDW$~mm, and ensemble spread $\sigma_\phi\approx\FITSIG^\circ$.
This reproduces both peaks with a reduced $\chi^2=\FITCHI$ (a single crack,
$\sigma_\phi=0$, gives $\FITCHISINGLE$). The strong $2f$ forcing splits the
diurnal cycle into two widening events, which at shallow depth produce two
distinct, well-separated emission peaks --- a prominent secondary near the
observed phase --- rather than the single dominant peak of a deep crack. An
affine-invariant MCMC over a fast flux emulator (Figure~\ref{fig:enc:post})
confirms the source is shallow ($L=\FITLPOST$~km) and the second harmonic strong,
though the emulator's accuracy in this strongly non-linear regime limits the
precision of the credible intervals.

There is a genuine tension worth stating plainly. A secondary-agnostic fit that
minimises only the pointwise $\chi^2$ instead prefers a \emph{deep}
($L\gtrsim20$~km), approach-peak-dominated crack whose secondary is a weak
shoulder --- a formally better $\chi^2$ but one that does not reproduce the
observed secondary \emph{peak}. Whether the second maximum is a genuine peak (our
adopted, prominence-weighted view) or a shoulder therefore controls the inferred
depth. The shallow, strong-$2f$ solution is the more physical of the two: the
reduced ice-shell thickness of the south-polar terrain favours a shallow source,
and the strong second harmonic is consistent with the multi-harmonic tidal stress
that drives the walls \cite{nimmo2014tidally}. Two caveats bound the inference:
the target is a digitized central-trend curve through a wide, multi-year scatter,
and the amplitude is a free scale, so the fit is driven by the profile shape and
the relative peak strengths, not the absolute emission.

\placeholder{Develop the implications: quote specific south-polar ice-shell
thickness estimates (gravity/topography inversions) consistent with
$L\approx\FITL$~km, and relate the strong fitted second-harmonic amplitude to
tidal-stress models of the tiger-stripe forcing.}
...
\begin{figure}[hbt!]
    ...
    Fig.~\ref{fig:enc:1}). The objective prioritises the \emph{relative strength}
    (prominence) of the secondary peak; the best fit is a shallow source
    ($L\approx\FITL$~km) with a strong second-harmonic (double-cosine) forcing
    ($\alpha\approx\FITTWOF$), which produces a distinct, prominent secondary peak
    near the observed phase. Free parameters are the tidal swing $\Delta w$, depth
    $L$, the $2f$ amplitude and phase, the along-strike ensemble spread
    $\sigma_\phi$, a global phase offset, and an amplitude (posterior in
    Figure~\ref{fig:enc:post}).}
    \label{fig:enc:fit}
\end{figure}
...
\begin{figure}[hbt!]
    ...
    MCMC over a fast flux emulator (1-D marginals with 16/50/84th percentiles;
    2-D joint densities; red crosses mark the exact maximum-likelihood point). The
    depth is robustly \emph{shallow} ($L=\FITLPOST$~km) and the second-harmonic
    amplitude strong, confirming the prominence-weighted best fit; the credible
    intervals are approximate, limited by the emulator's accuracy in this
    strongly non-linear (strong-$2f$) regime.}
    \label{fig:enc:post}
\end{figure}
